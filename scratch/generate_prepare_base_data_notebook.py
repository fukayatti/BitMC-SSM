...
def create_notebook():
    cells = [
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# 動作確認用の小規模設定。問題なければ ja_wiki_docs=8000 / ja_web_docs=24000 / en_docs=8000 程度に上げる\n",
                "# (49kの語彙を学習するのにWikipedia全体規模は不要。各ソースは{TOKENIZER_DIR}/_corpus_cache/にキャッシュされるので再実行は高速)。\n",
                "# ja_web_docs を他より多めにしているのは、固定語彙数の枠を口語日本語(スラング等)が\n",
                "# 英語/正式文体に食われて圧縮率が悪化した実測結果を踏まえた調整。\n",
