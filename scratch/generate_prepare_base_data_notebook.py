    ...
    cells = [
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "⚠️ **まずは小さいドキュメント数(下記デフォルト)でパイプライン全体が通ることを確認してから、本番規模に上げること。** 各ソースはストリーミング中に`{TOKENIZER_DIR}/_corpus_cache/`へ逐次テキストとしてキャッシュされ、2,000件ごとに進捗ログが出るので、途中で切断されても再ダウンロードなしで再開できる(進捗が見えず数分固まったように見えるのは仕様ではない)。`!`セルはエラーが出ても自動で次に進んでしまうため、次のアサーションセルで必ず成功を確認すること。\n",
                "\n",
                "⚠️ ドキュメント数を後で増やす場合は、**先に`_corpus_cache/`内の対応する`.txt`を削除**すること(キャッシュがあるとそちらが優先され、件数を増やしても再ダウンロードされない)。"
            ]
        },
        {
            "cell_type": "code",
            ...
            "outputs": [],
            "source": [
                "# 動作確認用の小規模設定。問題なければ ja_wiki_docs=20000 / ja_web_docs=20000 / en_docs=40000 程度に上げる\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# `!` セルはエラーで停止しないため、ここで明示的にファイルが生成されたか確認する。\n",
                "import os\n",
                "\n",
                ...
                ")\n",
                "print(f\"✅ Tokenizer files confirmed at {TOKENIZER_DIR}\")"
            ]
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "source": [
                ...
                ")\n",
                "print(f\"✅ {DATA_BIN} ({os.path.getsize(DATA_BIN) / (1024*1024):.2f} MB)\")"
            ]
        },
        {
            "cell_type": "markdown",
